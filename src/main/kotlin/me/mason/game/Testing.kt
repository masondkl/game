...
fun main() = window("Game", 1280, 720) {
    ...
    scene(sheet) {
        ...
        sword.rotatedBounds(
            0, //quad index in mesh
            position * 50f, //center point
//            vec(0f),
            vec(20f, 100f), // diameter
            vec(cos(rotation), sin(rotation)) * 50f, //pivot is 0 because we want to rotate around center
            rotation * (180f / PI).toFloat() + if (position.x > 0) 0f else 180f//rotation in degrees
//            0f
        )
